...
dE_dt = np.gradient(E_fwd_history, dt, axis=0)

eps_silicon = 3.48 ** 2
eps_air = 1.0 ** 2
deps_dp = eps_silicon - eps_air

E_adj_history_rev = E_adj_history[::-1]

integrand = E_adj_history_rev * deps_dp * dE_dt
gradient_exact = np.sum(integrand, axis=0) * dt * pixel_area
...
for idx_x in range(nx):
    ...
    for idx_y in range(ny):
        ...
        def record_fdm_efield(sim):
            ez_mon = sim.get_field_point(mp.Ez, monitor_position)
            e_monitor_fdm.append(ez_mon)

        sim_fdm.run(record_fdm_efield, until=T_f)
        E_monitor_fdm = np.array(e_monitor_fdm)

        J1 = 0.5 * np.sum(np.abs(E_monitor_fdm)**2) * dt
